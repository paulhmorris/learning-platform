...
const adminButtonVariants = cva(
  "inline-flex gap-1.5 items-center justify-center touch-manipulation whitespace-nowrap text-[15px] rounded-md sm:text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
      ...
      size: {
        default: "h-9 sm:h-8 px-3.5 py-2 sm:px-3 sm:py-1.5",
        sm: "h-7 px-3",
        lg: "h-10 px-8",
        icon: "size-9",
      },
...
const AdminButton = React.forwardRef<HTMLButtonElement, AdminButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return <Comp className={cn(adminButtonVariants({ variant, size, className }))} ref={ref} {...props} />;
  },
);
